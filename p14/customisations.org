...
** amdgpu module needed to load early

+Sometimes X wouldn't start.  Turns out, the amdgpu module needed to load early, earlier than kms.+
...
Now we are on wayland.  The issue with X cannot happen anymore.

* en_GB.UTF-8

Specified at install.

* /usr/share/dict

#+BEGIN_example
gargle@p14:~$ sudo apt remove --purge wamerican iamerican
gargle@p14:~$ sudo apt install wbritish
#+END_example

#+BEGIN_example
gargle@p14:~$ dpkg -l | grep -i american
ii  ienglish-common                       3.4.06-1                             all          Common files for British and American ispell dictionaries
gargle@p14:~$ dpkg -l | grep -i british
ii  ibritish                              3.4.06-1                             all          British English dictionary for ispell (standard version)
ii  ienglish-common                       3.4.06-1                             all          Common files for British and American ispell dictionaries
ii  wbritish                              2020.12.07-4                         all          British English dictionary words for /usr/share/dict
#+END_example

#+BEGIN_example
gargle@p14:~$ ls -haltr /usr/share/dict/
total 968K
-rw-r--r--   1 root root  199 Mar  2  2025 README.select-wordlist
-rw-r--r--   1 root root 955K Mar 15  2025 british-english
drwxr-xr-x 195 root root 4.0K Oct 25  2025 ..
lrwxrwxrwx   1 root root   30 May  9 17:54 words -> /etc/dictionaries-common/words
drwxr-xr-x   2 root root 4.0K May  9 17:54 .
gargle@p14:~$ ls -haltr /etc/dictionaries-common/words
lrwxrwxrwx 1 root root 31 May  9 17:55 /etc/dictionaries-common/words -> /usr/share/dict/british-english
#+END_example
...
#+BEGIN_example
gargle@p14:~$ ip addr show wlp2s0
4: wlp2s0: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc noqueue state UP group default qlen 1000
    link/ether 10:e1:8e:84:b0:d2 brd ff:ff:ff:ff:ff:ff
    altname wlx10e18e84b0d2
    inet 192.168.1.121/24 metric 1024 brd 192.168.1.255 scope global dynamic wlp2s0
       valid_lft 42749sec preferred_lft 42749sec
    inet6 2a02:1811:1415:9f80::270/128 scope global dynamic noprefixroute
       valid_lft 42751sec preferred_lft 42751sec
    inet6 fd92:d39a:76ce::270/128 scope global dynamic noprefixroute
       valid_lft 42751sec preferred_lft 42751sec
    inet6 fd92:d39a:76ce:0:12e1:8eff:fe84:b0d2/64 scope global mngtmpaddr noprefixroute
       valid_lft forever preferred_lft forever
    inet6 2a02:1811:1415:9f80:12e1:8eff:fe84:b0d2/64 scope global dynamic mngtmpaddr noprefixroute
       valid_lft 243521sec preferred_lft 70721sec
    inet6 fe80::12e1:8eff:fe84:b0d2/64 scope link proto kernel_ll
       valid_lft forever preferred_lft forever
#+END_example

*** figure out our nameserver
...
*** figure out the ip of the guest page
...
With a nameserver like 192.168.1.1 Librewolf would just go for the telenet dns servers instead.
...
#+BEGIN_example
gargle@p14:~$ sudo apt install ufw
#+END_example

We go with the defaults, just make sure IPV6=yes is on.

#+BEGIN_example
gargle@p14:~/pandora$ grep IPV6 /etc/default/ufw
IPV6=yes
#+END_example
...
#+BEGIN_EXAMPLE
gargle@p14:~$ sudo apt install vim
gargle@p14:~$ sudo apt remove vim-tiny --purge
#+END_EXAMPLE
...
<     bindsym $mod+Print exec grimshot save output /tmp/screenshot.png
<     bindsym $mod+Shift+Print exec grimshot save area /tmp/screenshot.png
<     bindsym $mod+Alt+Print exec grimshot save active /tmp/screenshot.png
<     bindsym $mod+Ctrl+Print exec grimshot save window /tmp/screenshot.png
<     # lock the screen
...
dc needs to be installed separately.  bc is installed because of zram-tools

#+BEGIN_example
gargle@p14:~$ sudo apt install dc
#+END_example
...
#+BEGIN_example
echo -e '\033]12;green\007'
#+END_example

** change background to black, foreground to white, and a green cursor

#+BEGIN_example
echo -e '\033]11;#000000\033]10;#ffffff\033]12;green\007'
#+END_example
...
#+BEGIN_example
gargle@p14:~$ sudo apt install pinentry-fltk
#+END_example

#+BEGIN_example
gargle@p14:~$ sudo apt remove pinentry-curses --purge
#+END_example
