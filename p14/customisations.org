...
The investigation is ongoing for systemd-networkd-wait-online.service!  For now, it is on.

#+BEGIN_example
root@p14:~# systemctl disable --now systemd-networkd-wait-online.service
root@p14:~# systemctl enable --now systemd-networkd-wait-online.service
#+END_example
...
* TODO this document
Make sure that comments in configuration files are shown, because xorg-mode filters out lines starting with #.
