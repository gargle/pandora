...
* issues
** mayor
Sometimes X wouldn't start.  Turns out, the amdgpu module needs to load early, earlier than
kms.

First we suspected networking, then xdm, then systemd-networkd-wait-online.sservice, but we
all keep those at the defaults.

** minor
Some characters appeared with less pixels.  Scaling seems to help.
...
** dhclient
How to figure out the dns name servers that were returned by dhclient, if you ever wonder why
our default cloudfare dns servers aren't used when connecting to a new access point without a
passphrase.
...
** dnsmasq
We use the cloudfare dns servers instead of our own, but we want to fall back on our own
router at home to resolve msi.lan, openwrt.lan, and any other device on our local lan.
...
** scaling
#+BEGIN_example
gargle@p14:~$ ls -haltr .local/bin/set-xrandr.sh
-rwxr-xr-x 1 gargle gargle 245 Oct 10 08:29 .local/bin/set-xrandr.sh
#+END_example

#+BEGIN_example
gargle@p14:~$ cat .local/bin/set-xrandr.sh
#!/bin/sh

# wait for output to appear
until /usr/bin/xrandr >/dev/null 2>&1 && /usr/bin/xrandr | grep -q '^eDP'; do sleep 0.5; done
/usr/bin/xrandr --output eDP --scale 1.25x1.25 --panning 2400x1500 --dpi 120 --pos 0x0 \
    >/dev/null 2>&1 || true
#+END_example

#+BEGIN_example
gargle@p14:~$ cat .config/systemd/user/xrandr-scale.service
[Unit]
Description=Apply xrandr scaling for eDP
After=graphical-session.target

[Service]
Type=oneshot
ExecStart=%h/.local/bin/set-xrandr.sh
Environment=DISPLAY=:0
# If you need XAUTHORITY, uncomment and adjust:
...
#+BEGIN_example
gargle@p14:~$ systemctl --user daemon-reload
gargle@p14:~$ systemctl --user enable --now xrandr-scale.service
#+END_example
...
We start emacs on the right side of the screen by default.
...
** menu
#+BEGIN_example
gargle@p14:~/pandora$ cat ~/GNUstep/Defaults/WMRootMenu
(
    Applications,
    (Exit, EXIT),
...
** Preferences
#+BEGIN_example
gargle@p14:~$ diff GNUstep/Defaults/WindowMaker GNUstep/Defaults/WindowMaker.old | grep Trebu
<   MenuTitleFont = "Trebuchet MS,Luxi Sans:weight=200:pixelsize=16";
>   MenuTitleFont = "Trebuchet MS,Luxi Sans:bold:pixelsize=12";
<   WindowTitleFont = "Trebuchet MS,Luxi Sans:weight=200:pixelsize=16";
>   WindowTitleFont = "Trebuchet MS,Luxi Sans:bold:pixelsize=12";
<   MenuTextFont = "Trebuchet MS,Luxi Sans:pixelsize=16";
